...
post '/backend' do
    ...
    artist =""
    body=""
    #loop through all the text messages  I have recieved
    @client.account.messages.list.each {|message|
        
        #parse the message body with regulare expressions
        message.body=~/(^[\s\S]*)by(.*)/
        song=$1
        artist=$2
        #grab row that has the same song value
        checkexists = con.query "SELECT song FROM votetable WHERE song='#{song}';"
         
        if "#{checkexists.fetch_row}" != "[\"#{song}\"]"
            #store song into row if it hasnt been stored before
    	   puts "inserted"
